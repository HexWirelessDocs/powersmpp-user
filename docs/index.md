---
password: ''
---

# Welcome to Power SMPP User Guide

Power SMPP is a high-performance and reliable messaging gateway designed for telecom operators, aggregators, and enterprises.

This documentation covers the user-facing features including messaging, contacts, reporting, and developer APIs.

For more information, visit the official website: [Power SMPP.com](https://www.powersmpp.com){ target="_blank" }

---

## Explore the Documentation

<div class="grid cards" markdown>

-   :material-account:{ .lg .middle } **User Guide**

    ---

    Access features for end users including messaging and reporting

    [:octicons-arrow-right-24: Get Started](user/dashboard/dashboard.md)

-   :material-book-open-variant:{ .lg .middle } **Glossary**

    ---

    Quick reference guide to common terms used in the documentation

    [:octicons-arrow-right-24: View Glossary](glossary.md)

-   :material-frequently-asked-questions:{ .lg .middle } **FAQ**

    ---

    Find quick answers to common questions about Power SMPP

    [:octicons-arrow-right-24: View FAQ](faq.md)

</div>

---

## Let's Discuss a Solution for You

!!! quote "Contact Us"
    :material-phone: Call us directly: [+91-9909945175](tel:+919909945175)
    :material-whatsapp: Or [Click here to WhatsApp](https://wa.me/919909945175){ target="_blank" }
